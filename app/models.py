...
class UserDetails(models.Model):
    ...
    station = models.ForeignKey(
        stationDetails, null=True, blank=True, on_delete=models.SET_NULL)
...
class AssetSubCategory(models.Model):
    ...
    category = models.ForeignKey(
        AssetCategory, null=True, blank=True, on_delete=models.SET_NULL)
...
class Asset(models.Model):
    ...
    asset_category = models.ForeignKey(
        AssetSubCategory, null=True, blank=True, on_delete=models.SET_NULL)
    ...
    assign_to = models.ForeignKey(
        UserDetails, null=True, blank=True, on_delete=models.SET_NULL)
    ...
    def save(self, *args, **kwargs):
        ...
        try:
            existing_asset = Asset.objects.get(
                pk=self.pk)  # Fetch the previous state
...
class Allocation(models.Model):  # issuedproducts
    ...
    user = models.ForeignKey(UserDetails, null=True,
...
class Maintenance(models.Model):
    ...
    last_maintenance_date = models.DateField(
        null=True, blank=True)  # Tracks the last maintenance date
    next_maintenance_date = models.DateField(
        null=True, blank=True)  # Tracks the next maintenance date
    maintenance_cost = models.CharField(max_length=255, null=True, blank=True)
    # Add this field to capture return date
    return_date = models.DateField(null=True, blank=True)
...
class ExpiredProduct(models.Model):
    ...
    asset = models.OneToOneField(
        "Asset", on_delete=models.CASCADE, related_name="expired_record")
    expiration_date = models.DateField(auto_now_add=True)
    reason = models.CharField(
        max_length=255, default="Expired status set by admin")  # Optional field
...
class StockHistory(models.Model):
    ...
    def _str_(self):
        return f"{self.asset.asset_name} - {self.stock_level} on {self.date}"
